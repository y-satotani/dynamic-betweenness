...
void decremental_update(igraph_t* G,
  ...
  if(weights) {
    queue = malloc(sizeof(igraph_2wheap_t));
    igraph_2wheap_init((igraph_2wheap_t*)queue, igraph_vcount(G));
  } else {
    queue = malloc(sizeof(rheap_t));
    rheap_init((rheap_t*)queue, igraph_vcount(G), 0);
  }
  ...
  for(igraph_integer_t si = 0; si < igraph_vector_int_size(&sources); si++) {
    ...
    if(weights)
      update_betw_dec_weighted_statistics
        (G, &preds, D, S, B, u, v,
         ...
         &is_affected, (igraph_2wheap_t*)queue, &Delta, &aff_deps_before);
    else
      update_betw_dec_unweighted_statistics
        (G, &preds, D, S, B, u, v,
         s, &targets, -factor, 0,
         &is_affected, (rheap_t*)queue, &Delta, &aff_deps_before);
    end = clock();
    if(upd_stats) {
    ...
    if(weights)
      update_sssp_dec_weighted
        (G, &preds, &succs, D, S, u, v, s, &targets, weights, weight, 0,
         &is_affected, (igraph_2wheap_t*)queue);
    else
      update_sssp_dec_unweighted
        (G, &preds, &succs, D, S, u, v, s, &targets, 0,
         &is_affected, (rheap_t*)queue);
    end = clock();
    if(upd_stats) {
    ...
    if(weights)
      update_betw_dec_weighted_statistics
        (G, &preds, D, S, B, u, v,
         ...
         &is_affected, (igraph_2wheap_t*)queue, &Delta, &aff_deps_after);
    else
      update_betw_dec_unweighted_statistics
        (G, &preds, D, S, B, u, v,
         s, &targets, factor, 1,
         &is_affected, (rheap_t*)queue, &Delta, &aff_deps_after);
    end = clock();
    if(upd_stats) {
      double time_betw = (double)(end - start) / CLOCKS_PER_SEC;
      upd_stats->time_full += time_betw;
  ...
  if(weights) {
    igraph_2wheap_destroy((igraph_2wheap_t*)queue);
  } else {
    rheap_destroy((rheap_t*)queue);
  }
  free(queue);
